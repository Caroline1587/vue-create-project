  ...
  >
    <ul class="folderMenuList">
      <li
        ...
        :key="item.cmd"
        :disabled="currentNode?.level === item.flag||!props.role[item.roleLabel]||currentNodeData?.isFullPermission === 0"
...
import { computed, nextTick, ref, watch } from 'vue'
  ...
  const emits = defineEmits(['deleteNode', 'checkNode', 'nodeClick', 'exportOrImport'])
  // 树形节点数据
  ...
  const handleTree = () => {
    ...
    if (cmd === 'addFolder') {
      if (currentNode.value.level >= 10) {
        ...
        errorMessage('wenJianJiaCengJiShenDuXianZhiZuiDaWei_10CengDangQianJieDianWuFaXinJianZiJieDian')
        return
      }
  ...
  watch(() => props.folderExpand, newValue => {
